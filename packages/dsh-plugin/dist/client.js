...
export function resolveKernlClientConfig(input) {
    const config = input === undefined ? {} : input;
    if (!isJsonObject(config))
        throw new TypeError('kernl-service config must be an object');
    ...
    if (typeof baseUrl !== 'string')
        throw new TypeError('kernl-service config.baseUrl must be a string');
    let parsedBase;
    try {
...
export function createKernlClient(input, fetchImplementation = globalThis.fetch) {
    ...
    const post = async (route, request, callerSignal) => {
...
function endpointFor(config, route) {
