import { isJsonObject, type CordisContextLike, type DshToolsLike, type JsonObject } from './contracts.js'
...
export const inject = ['tools', 'kernl', 'kernlTools', 'appExit'] as const
...
export function apply(ctx: CordisContextLike, config: SmokeConfig = {}): void {
  const tools = ctx.get('tools') as DshToolsLike | undefined
  const appExit = ctx.get('appExit')
  if (tools === undefined || typeof tools.execute !== 'function') {
    throw new TypeError('kernl-smoke-probe requires ctx.tools.execute()')
  }
  if (typeof appExit !== 'function') throw new TypeError('kernl-smoke-probe requires ctx.appExit')
  ...
  if (typeof ctx.effect !== 'function') throw new TypeError('kernl-smoke-probe requires ctx.effect()')
